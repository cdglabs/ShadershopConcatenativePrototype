...
(function() {
  var AddLinkView, AnnotateMixin, Apply, ApplyInternalsView, ApplyRowView, ApplyThumbnailView, ApplyView, ContentEditableMixin, DataForMixin, DraggingView, Editor, Fn, Graph, GraphView, LinkRowView, LinkThumbnailView, LinkView, MainGraphView, Param, ParamTitleView, ParamValueView, ParamView, PossibleApplyView, ProvisionalApply, ProvisionalApplyView, R, compose, config, cx, drawLine, editor, fnsToAdd, lastHoveredEls, lerp, onceDragConsummated, pointermove, pointerup, refresh, refreshView, ticks, truncate, updateHover, updateHover2, _base, _ref, _ref1,
  ...
  ApplyInternalsView = React.createClass({
    render: function() {
      ...
      }, apply instanceof Param ? ParamView({
        param: apply
      }) : [
        R.div({
          className: "fnTitle"
  ...
  PossibleApplyView = React.createClass({
    ...
    annotate: function() {
      if (this.props.possibleApply.params.length > 1) {
        return {
          self: {
            hoverParam: this.props.possibleApply.params[1]
          }
        };
      }
    },
    handleHoverEnter: function() {
      return this.props.apply.selectedApply = this.props.possibleApply;
    },
    handleHoverLeave: function() {
      return this.props.apply.selectedApply = null;
    },
  ...
  DraggingView = React.createClass({
    render: function() {
      var _ref;
      if ((_ref = editor.dragging) != null ? _ref.render : void 0) {
        return R.div({
  ...
  */
      ...
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        ...
        if (data.apply instanceof Param && data.apply !== editor.xParam) {
          ...
          if (data.apply.axis === "x") {
            _results.push(graph.drawVerticalLine(graphFn(0), data.styleOpts));
          } else if (data.apply.axis === "result") {
            _results.push(graph.drawHorizontalLine(graphFn(0), data.styleOpts));
          } else {
            _results.push(void 0);
          }
        } else {
  ...
  LinkView = React.createClass({
    ...
    handleMouseDown: function(e) {
      var chain, el, link, offset, rect, _ref,
        _this = this;
      if (e.target.closest(".param") != null) {
        return;
      }
  ...
  updateHover = function() {
    ...
    while ((el != null ? el.nodeType : void 0) === Node.ELEMENT_NODE) {
      if (editor.hoveredApply == null) {
        editor.hoveredApply = (_ref = el.annotation) != null ? _ref.hoverApply : void 0;
      }
      if (editor.hoveredParam == null) {
        editor.hoveredParam = (_ref1 = el.annotation) != null ? _ref1.hoverParam : void 0;
      }
  ...
  updateHover2 = function() {
    var el, hoveredEl, hoveredEls, lastHoveredEl, _base, _i, _j, _len, _len1, _ref;
    if (editor.dragging) {
      return;
    }
    hoveredEls = [];
    el = document.elementFromPoint(editor.mousePosition.x, editor.mousePosition.y);
    while ((el != null ? el.nodeType : void 0) === Node.ELEMENT_NODE) {
      if ((_ref = el.dataFor) != null ? _ref.handleHoverEnter : void 0) {
        hoveredEls.push(el);
      }
      el = el.parentNode;
    }
    for (_i = 0, _len = lastHoveredEls.length; _i < _len; _i++) {
      lastHoveredEl = lastHoveredEls[_i];
      if (!_.contains(hoveredEls, lastHoveredEl)) {
        if (typeof (_base = lastHoveredEl.dataFor).handleHoverLeave === "function") {
          _base.handleHoverLeave();
        }
      }
    }
    for (_j = 0, _len1 = hoveredEls.length; _j < _len1; _j++) {
      hoveredEl = hoveredEls[_j];
      if (!_.contains(lastHoveredEls, hoveredEl)) {
        hoveredEl.dataFor.handleHoverEnter();
      }
    }
    return lastHoveredEls = hoveredEls;
  };
  ...
  ProvisionalApply = (function() {
    ...
    ProvisionalApply.prototype.compileString = function() {
      if (this.selectedApply) {
        return this.selectedApply.compileString();
      } else {
        return this.params[0].compileString();
      }
    };
